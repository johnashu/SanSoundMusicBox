...
library SafeERC20 {
    ...
    function safeTransfer(IStuckERC20 token, address to, uint256 value) internal {
...
interface SANSoulbindable {
    ...
    event SoulbindingEnabled(bool isEnabled);
...
contract SANOrigin is SAN721, SANSoulbindable {
    ...
    bytes32 public constant __XIN__ = keccak256(
        abi.encodePacked(
            /*                              \???/
                                     \?/
                                      '
            ...
            ___SAN___
                                    ...
                                    \333/
                                     \3/
                                      '
        */
    uint256 public constant _S_O_R_A_ = (
        (
            (
                (((((0x000e77154) << 33 | 0x0de317498) << 33 | 0x1d07b6070) << 33 | 0x1f061e54f) << 33 | 0x14bf0daef)
                    << 33 | 0x16635c817
            ) << 33 | 0x0ad6c9a0b
        ) << 33 | 0x199a0adf2
    );
    uint256 public constant MAX_LEVEL_FOUR_SOULBINDS = uint256(__XIN__) ^ _S_O_R_A_;
    ...
    function _The_static_percolates_our_unlit_sky___() external pure returns (bytes32 n) {
        n = hex"734a4e6b3179";
    }

    function __Still_tension_is_exhausted_by_our_pain___() external pure returns (bytes32 m) {
        m = hex"7068617634696e";
    }

    function setSoulbindingEnabled(bool _isEnabled) external onlyOwner {
        soulbindingEnabled = _isEnabled;
        emit SoulbindingEnabled(_isEnabled);
    }

    function ___As_a_warm_purr_prepares_to_amplify___() external pure returns (bytes32 l) {
        l = hex"614a6d31706c6956664479";
    }

    function ____Our_apprehensions_cross_a_sonic_plane___() external pure returns (bytes32 k) {
        k = hex"706e6c61666e7265";
    }

    function addUserSoulbindCredits(address[] calldata _accounts, uint256[] calldata _credits) external onlyOwner {
        unchecked {
    ...
    function _____Initiating_first_transmissions_now___() external pure returns (bytes32 j) {
        j = hex"6e46466f5777";
    }

    function ______At_last_our_pitch_black_planet_twinkles_to___() external pure returns (bytes32 i) {
        i = hex"744a4c6f6f";
    }

    function setLevelPrices(uint256[] calldata _newPrices) external onlyOwner {
    ...
    function _______We_waited_for_permission_to_avow___() external pure returns (bytes32 h) {
        h = hex"6132766f4c3577";
    }

    function ________That_seizing_silence_take_an_altered_hue___() external pure returns (bytes32 g) {
        g = hex"686145756e65";
    }
    ...
    function userMaxSoulboundLevel(address _owner) external view returns (SoulboundLevel) {
        ...
        unchecked {
            for (uint256 i; i < tokenCount; i++) {
                SoulboundLevel level = tokenLevel[tokenOfOwnerByIndex(_owner, i)];
    ...
    function _________Baptized_to_the_tune_of_our_refound_rite___() external pure returns (bytes32 f) {
        f = hex"72694a74345665";
    }

    function __________Though_mute_shade_has_reborn_our_infancy___() external pure returns (bytes32 e) {
        e = hex"696e516678616e63546779";
    }
    ...
    function ___________We_rise_from_ruins_of_eternal_night___() external pure returns (bytes32 d) {
        d = hex"6e4869674c683174";
    }

    function ____________Saved_solely_by_Suncore_Light_Industry___() external pure returns (bytes32 c) {
        c = hex"496e4d7364754c7374727779";
    }
    ...
    function _____________FOR_YEARS_OUR_SENSES_WERE_UNDER_ATTACK___() external pure returns (bytes32 DIC) {
        DIC = hex"4150545054704143514b";
    }

    function ______________UNTIL_NEW_SENSORS_WERE_TRANSPORTED_BACK___() external pure returns (bytes32 sfpi) {
        sfpi = hex"4250416d43514b";
    }
} //                             三
